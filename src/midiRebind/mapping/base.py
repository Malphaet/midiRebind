# This is the basic mapping of a midi message, all subsequent messages must either be children of this or implement
# those methods
import configparser
...
class BasicMessageParse(object):
    ...
    def __init__(self):
        '''Basic init'''
        self._mapping = {"value": None,
                         "type": None}  # A barebone initialisation, unusable, it will need to be overridden to work
...
class BasicActions(object):
    ...
    def makeOneTrigger(self, keys, key_i, special, actions_t, startkey, stopkey):
        ...
        if key_i not in keys:  # Must now initialise
            ...
            if special in ["val", "toggle"]:  # Only do the function once # TODO: ADD FUNCTION WITH MESSAGE
                ...
                keys[key_i] += self.makeAllKeys(key_i, actions_t, startkey=startkey,
                                                stopkey=stopkey)  # Do a different trigger for every ; and link to a different interface for every n/
    ...
    def makeAllKeys(self, key, action, startkey=0, stopkey=0):
        ...
        try:
            interF = self.interface.interface_out(int(interfaceout))
            noteN = self.findNote(key, startkey, stopkey, note)
            return [BasicMidiTrigger(interF, self, midiaction, noteN, intensity, binded, key)]
        except (KeyError):
            print(
                "[WARNING] Can't find interface {} for note {} action {}, legal interfaces are {}".format(interfaceout,
                                                                                                          note, action,
                                                                                                          [i for i in
                                                                                                           self.interface.outputs.keys()]))
    ...
    def findNote(self, key, startkey, stopkey, note):
        ...
        if startkey != stopkey:  # do an interpolation over the values
            ...
            if "-" in note:  # Bind ALL notes to a single one, this is not doable this way
                print(
                    "[WARNING] As of now it is not possible to bind one note to severall output notes, ({})->({})".format(
                        note, key))
...
class BasicMidiTrigger(object):
    ...
    def execspecialfn(self, val):
        ...
        try:
            dprint("[Sent:({}.)]: Note {} / Parameters : {}".format(self.interface_short_name, self.received_note,
                                                                    self.params))
            # self.funct(self.params)

            if self.funct:
                dprint(
                    "[Sending function {} - Note {} / Parameters : {}".format(self.funct.__name__, self.received_note,
                                                                              self.params))
...
class EmptyTrigger(BasicMidiTrigger):
    ...
    def __call__(self, val):
        "Send the message"
        dprint("[Sending function (call) {} - Note {} / Parameters : {}".format(self.funct.__name__, self.received_note,
                                                                                self.params))
        self.execspecialfn(val)
    ...
    def sendmessage(self, val):
        ...
        dprint("[Sending function (send) {} - Note {} / Parameters : {}".format(self.funct, self.received_note,
                                                                                self.params))
        self.execspecialfn(val)
