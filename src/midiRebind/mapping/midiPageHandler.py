...
class midiPageHandler(object):
    ...
    def initValues(self):
        ...
        self._basevalues = [[[0 for i in range(self._widthV)] for j in range(self._heightV)] for k in
                            range(self._maxpages)]
...
class pulseRackController(object):
    ...
    def __repr__(self):
        return ("PulseRackController{} ({self.idlastlayerpressed},{self.idlastmemorypressed})".format(
            list(self.ranges.keys()), self=self))
...
class AkaiAPCMini(midiPageHandler):
    ...
    def __init__(self):
        super(AkaiAPCMini, self).__init__(
            [[i + (7 - j) * 8 for i in range(8)] for j in range(8)] + [[i for i in range(64, 72)],
                                                                       [i for i in range(82, 90)]])
        # Add the two control lines
        self.initValues()

        self.colors = {"black": 0, "green": 1, "blinking_green": 2, "red": 3, "blinking_red": 4, "yellow": 5,
                       "blinking_yellow": 6}
        self.addPossibleValues(("Inactive", "Active", "Selected", "Live"))
        _INACTIVE, _ACTIVE, _LIVE, _SELECTED = self.statusId("Inactive"), self.statusId("Active"), self.statusId(
            "Live"), self.statusId("Selected")
    ...
    def noteReceived(self, note, val):
        ...
        try:
            ...
            wprint("List of modules available on page {page} is : {modules}".format(page=self._page,
